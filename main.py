...
def execution(file_path: str, problem: int) -> int:
    ...
    for idx in range(length):
        ...
        with open(input_list[idx], "r") as f_in, open(output_list[idx], "r") as f_out:
            input_data = f_in.read()
            except_output = f_out.read()
        #print(f"input : {input_data}, output : {except_output}")
        execution_result = subprocess.run(execution_command, input=input_data.encode(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
